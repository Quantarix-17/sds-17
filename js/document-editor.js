...
function fitEditorPagesToScreen() {
  ...
  const isNarrow = typeof isMobileDeviceLayout === 'function'
    ? isMobileDeviceLayout()
    : (window.innerWidth <= 850);

  // Desktop / wide: native A4 size, no scaling
  if (!isNarrow) {
    pages.forEach(p => {
      p.style.zoom = '';
      p.style.transform = '';
  ...
  // Mobile / narrow: scale A4 pages to fit the panel width while staying readable
  const containerWidth = docContainer.clientWidth || 0;
  ...
  // Prefer the actual container width; fall back to viewport. Keep tiny side gutters only.
  const gutter = 8;
  const available = Math.max(1, (containerWidth > 40 ? containerWidth : viewportWidth) - gutter);
  // Floor ~0.42 keeps text usable on 320px; target is near full width
  let scale = available / EDITOR_A4_WIDTH;
  scale = Math.min(1, Math.max(0.42, scale));
  // Prefer slightly larger when we have room (avoid over-shrinking due to scrollbars)
  if (available >= 360) scale = Math.max(scale, Math.min(1, (available - 4) / EDITOR_A4_WIDTH));

  const scaledHeight = Math.round(EDITOR_A4_HEIGHT * scale);
  const gap = Math.max(12, Math.round(16 * scale));

  pages.forEach(p => {
    // Prefer transform over zoom — consistent across browsers; layout compensated via margin
    p.style.zoom = '';
    p.style.width = EDITOR_A4_WIDTH + 'px';
    p.style.height = EDITOR_A4_HEIGHT + 'px';
    p.style.minHeight = EDITOR_A4_HEIGHT + 'px';
    ...
    // Collapse the leftover layout space under the scaled page
    p.style.marginBottom = (scaledHeight - EDITOR_A4_HEIGHT + gap) + 'px';
...
// Debounced refit on orientation / resize (mobile)
