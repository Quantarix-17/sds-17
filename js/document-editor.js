...
function fitEditorPagesToScreen() {
  ...
  // Desktop / wide: native A4, no scaling
  if (!isNarrow) {
    pages.forEach(p => {
      p.style.zoom = '';
      p.style.transform = '';
    ...
    docContainer.style.alignItems = '';
    return;
  }

  // Force container to use full panel width and start from the left
  ...
  const containerWidth = Math.max(
    1,
    Math.floor(rect.width || docContainer.clientWidth || window.innerWidth || 1)
  );
  // 6px total horizontal breathing room
  const available = Math.max(1, containerWidth - 6);
  ...
  // Center the scaled page inside the container
  const offsetX = Math.max(0, Math.round((containerWidth - scaledW) / 2));
  const gap = Math.max(14, Math.round(18 * scale));
  ...
  pages.forEach(p => {
    ...
    // Scale from top-left, then shift so the visual page is centered
    p.style.transformOrigin = 'top left';
    p.style.transform = 'translateX(' + offsetX + 'px) scale(' + scale + ')';
    p.style.marginLeft = '0';
    p.style.marginRight = '0';
    // Collapse unused layout height under the scaled visual
    p.style.marginBottom = (scaledH - EDITOR_A4_HEIGHT + gap) + 'px';
  });
  docContainer.style.setProperty('--editor-page-scale', String(scale));
}

// Debounced refit on orientation / resize / visualViewport (mobile keyboard etc.)
if (!window.__editorPageFitResizeBound) {
  ...
  // Refit when switching to Editor tab on mobile
  document.addEventListener('click', (e) => {
    const t = e.target;
    if (!t || !t.closest) return;
    if (t.closest('#mob-btn-editor, #tab-editor-btn, #tab-editor-btn-desktop')) {
